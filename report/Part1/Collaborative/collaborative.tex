...
Now that we have found to users (b and c), that have rated items similar to user a, we are able to proceed and predicting rating for the items, user a not yet have rated. According to \todo{kilde til RS, an Introduction, p16} one possible way of making the prediction is based on this formula:\\
...
Differently from the Pearson method, does the formula not take into account that users use a rating scale differently. \todo{maybe note the adjusted cosine measure her? p19}
