...
\begin{itemize}
	\item \underline{Students} are interacting with the output of the system
	\item \underline{Teachers} are writing and changing the course descriptions and will therefore have opportunity to alter the output of the system
%Our initial thought was that the teachers are responsible for the course descriptions which defines the keywords for the individual courses, and hence have an impact on the system. We have subsequently discussed whether a teachers actually were a direct stakeholder, because our system only collects the data information from ITUs servers, and the teachers therefore do not have any direct interaction with the system. The reason we finally decided that they are part of the direct stakeholders is that by , hence we believe they are to be seen as a direct stakeholder)
	\item \underline{Persons} that will be in charge of determining the keywords for the system.
%They have an impact on the system in relation to the non-keywords they type in, because these can potentially alter the outcome of a keyword analysis, hence changing the relevance of a specific course.
\end{itemize}

\textbf{Indirect stakeholders}
\begin{itemize}
	\item \underline{Teachers} are indirectly impacted in regards to whether students choose to sign up for their course(s) or not, because the course can potentially be canceled if there are not a minimum number of sign-ups.
	\item \underline{Teachers assistants} are impacted indirectly the same way as teachers.
%	\item ITU.
...
This section describes the benefits and harms for the system's stakeholders.
We recognized the different benefits and harms through a discussion following the definition of our stakeholders. \newline

\textbf{Benefits for direct stakeholders}
\begin{itemize}
	\item \underline{Students} benefit from being given suggestions on behalf of their previous selected elective courses, which can support them in finding their next elective course(s).
	\item \underline{Teachers} benefit by being able to specify very precisely what the course is about and then getting students which prefer these preferences.
\end{itemize}

\textbf{Benefits for indirect stakeholders}
\begin{itemize}
	\item \underline{Teachers} benefit of students being recommended their courses, which will potentially help reach the minimum number of students of the course
	\item \underline{Teachers assistants} benefit from the same as the teachers does, in that when the minimum student limit is reached, they have a job for the following semester.
\end{itemize}

\textbf{Harms for direct stakeholders}
\begin{itemize}
	\item \underline{Students} who do not have any previous elective courses, will give the system no information to base the recommendations on.
	\item \underline{Teachers}
...
They can also be harmed if the producers of the non-keyword list adds a non-keyword that is essential to one or many of the teachers courses, since this will result in the course in some cases will not be taken into account.
	\item \underline{Persons} responsible for the keyword list, can accidentally add a wrong keyword to the list, which will change the outcome of the recommendations. This can give the person a bad reputation.
\end{itemize}

\textbf{Harms for indirect stakeholders}
\begin{itemize}
	\item \underline{Teachers} could experience students joining their course, based upon a low quality recommendation. This could make the student negatively influence the class sessions of the course. 
	\item \underline{Teachers assistants} could experience the same as the teachers.
\end{itemize}

As can be seen above, there are a variety of both benefits and harms that can affect the stakeholders of the system. In the next section we will have a look at our chosen values in \todo{in relation the the benefits and harms?}
...
% Harms
% \begin{itemize}
% 	\item Because the students are only recommended courses based on previous chosen ones, this will remove the diversity of the recommender system.
% \end{itemize}
...
%emphasis on changing the ÒcommonÓ way systems are built, where biases are found and dealt with after the system has been developed, and instead working towards a mentality where we see the elimination of biases in software systems as an integrated part of the development process.
